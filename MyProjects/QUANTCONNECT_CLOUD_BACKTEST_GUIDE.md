...
Example: `https://www.quantconnect.com/project/12345678`
...
### 5. Dismiss the Import Statements Popup

**Every time** the project loads or refreshes, a popup appears:

> "We recommend adding import statements to your code for a better coding experience. Would you like us to add this code for you?"

**Always dismiss this popup** before proceeding with any other actions. The popup has three clickable elements: "Cancel" button, "Yes" button, and an "X" close button in the top-right corner of the dialog. The most reliable way to dismiss it is to click the **X** button (the "Cancel" button refs can be hard to target with `find`). Always dismiss — never click "Yes".

### 6. Wait for IDE to Load
...
## IDE Layout and Navigation

### Right Ribbon (Vertical Icon Bar)

The QuantConnect IDE has a thin vertical icon ribbon on the **far right edge** of the screen. These icons toggle panels that overlay the right side of the editor. The top icon is the **Explorer** button.

**Important**: The Mia AI Assistant panel can cover the right ribbon. If you don't see the ribbon, close Mia first by clicking elsewhere or toggling it off via the "ASK MIA" button in the top-right toolbar area.

The Explorer icon is located at approximately `(1489, 113)` on a standard 1512-wide viewport. Click it to toggle the Explorer panel open/closed.

### Explorer Panel

The Explorer panel opens on the right side and shows a **WORKSPACE (WORKSPACE)** file tree. A typical project contains:

- `project/` — subfolder (may contain additional modules)
- `models/` — subfolder for model classes
- `research/` — subfolder containing research notebooks (`.ipynb` files)
- `main.py` — the main algorithm file (shown at root level)

Below the file tree, the Explorer also shows:

- **OUTLINE** — a code outline of the currently open file, listing classes, methods, and instances
- **TIMELINE** — version/change history

To open a file, click on it in the Explorer file tree. To expand a folder (e.g., `research/`), click on the folder name.

### Left Sidebar

The left sidebar has two states — collapsed (showing only icons) and expanded (showing icons + labels). Toggle between them using the `<<` / `>>` arrows at the bottom of the sidebar.

**Left sidebar icons (top to bottom):**

- **Home** — navigate to QuantConnect home
- **Code `<>`** — current project view (highlighted when in project IDE)
- **Notebook icon** — (appears to be for research/notebooks)
- **Deploy/rocket icon** — deployment related
- **Strategy icon** — strategies
- **People icon** — collaboration

When expanded, the left sidebar shows the **PROJECT** panel with:

- Project name and description
- Close, Clone, Migrate, Delete actions
- **LIBRARIES** — manage project libraries
- **COLLABORATE** — add collaborators
- **PARAMETERS** — add algorithm parameters
- **ENCRYPTION** — encrypt the project
- **LEAN ENGINE** — shows the current Lean engine version
